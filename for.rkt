...
(define-syntax-parse-rule (for/string (~optional (~seq #:length slen:expr) #:defaults ([slen #'32])) clauses body ... tail-expr)
...
(define-syntax-parse-rule (for/bytes (~optional (~seq #:length slen:expr) #:defaults ([slen #'32])) clauses body ... tail-expr)
  #:with original this-syntax
  #:with ((pre-body ...) (post-body ...)) (split-for-body this-syntax #'(body ... tail-expr))
  (let ([rlen slen])
